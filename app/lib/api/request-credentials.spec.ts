...
import { getApiKeysFromRequest, getApiKeysFromRequestHeaders, getProviderSettingsFromRequest, getProviderSettingsFromRequestHeaders } from './request-credentials';
...
describe('request-credentials helpers', () => {
  ...
  it('prefers explicit headers over cookies', () => {
    const request = {
      headers: new Headers({
        Cookie: 'apiKeys=%7B%22legacy%22%3A%22secret%22%7D; providers=%7B%22anthropic%22%3A%7B%22foo%22%3A%22bar%22%7D%7D',
  ...
  it('falls back to cookie values when no headers are present', () => {
    const request = {
      headers: new Headers({
        Cookie: 'apiKeys=%7B%22openai%22%3A%22cookie-key%22%7D; providers=%7B%22openai%22%3A%7B%22mode%22%3A%22fast%22%7D%7D',
      }),
    };
